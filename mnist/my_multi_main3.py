...
class Net(nn.Module):
    def __init__(self,max_type,layer_norm_flag,lam,gamma,head_cnt):
        ...
        self.attention_pool = [ConvAddAttention(20,int(500/head_cnt),12,1,max_type,layer_norm_flag,lam,gamma) for _ in range(head_cnt)]
...
def main(args = None):
    ...
    if (args.save_model):
        torch.save(model.state_dict(),"mnist3_multi_%2.1f_%.4f_%s_%s_%d_%.2f_%.2f.pt"%(test_acc*100,args.lr,args.optim_type,args.max_type,args.norm_flag,args.lam,0.123 if args.gamma is None else args.gamma))
