...
class Net(nn.Module):
    def __init__(self,max_type,layer_norm_flag,lam,gamma):
        ...
        self.attention_pool = ConvAddAttention(20,500,12,1,max_type,layer_norm_flag,lam,gamma)
    ...
    def forward(self, x):
        ...
        # x = x.view(-1, 4*4*50)
        # x = F.relu(self.fc1(x))
        x = F.relu(self.attention_pool(x).view(-1,500))
        x = self.fc2(x)
...
def main(args = None):
    ...
    model = Net(args.max_type,args.norm_flag,args.lam,args.gamma).to(device)
    ...
    if (args.save_model):
        torch.save(model.state_dict(),"mnist3_%2.1f_%.4f_%s_%s_%d_%.2f_%.2f.pt"%(test_acc*100,args.lr,args.optim_type,args.max_type,args.norm_flag,args.lam,0.123 if args.gamma is None else args.gamma))
